...
public class PacePalController {
	...
	public Song getSongById(@PathVariable Long songId) {
		...
		return pacepalService.retrieveSongById(songId);
	}

	// get tag by ID ("tagId)
	@GetMapping("/tags/{tagId}")
	public Tag getTagById(@PathVariable Long tagId) {
		log.info("Retrieving tag with ID {}", tagId);
		return pacepalService.retrieveTagById(tagId);
	}

	/*
	 * @PutMapping("/songs/{songId}") public PacepalSong updateSong(@PathVariable
	 * Long songId, @RequestBody PacepalSong pacepalSong) {
	 * pacepalSong.setSongId(songId);
	 * 
	 * System.out.println("Updating song with ID: " + songId);
	 * 
	 * return pacepalService.saveSong(pacepalSong); }
	 */
}