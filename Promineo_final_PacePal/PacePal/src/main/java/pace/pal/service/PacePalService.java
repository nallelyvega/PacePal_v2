...
public class PacePalService {
	...
	public void copyTagFields(Tag tag, PacepalTag playlistTag) {
		...
		tag.setPlaylistId(playlistTag.getPlaylistId());
	}

	public PacepalTag saveTag(Long playlistId, PacepalTag tagData) {
		Playlist playlist = findOrCreatePlaylist(playlistId);
		Tag tag = findOrCreateTag(playlistId, tagData.getTagId());
		copyTagFields(tag, tagData);
	...
	// retrieve songs by songId
	@Transactional(readOnly = true)
	public Song retrieveSongById(Long songId) {
		return songDao.findById(songId).orElseThrow(() -> new NoSuchElementException("Song with ID not found"));
	}

	// update song by songId

	// retrieve tags by songId
	@Transactional(readOnly = true)
	public Tag retrieveTagById(Long tagId) {
		return tagDao.findById(tagId).orElseThrow(() -> new NoSuchElementException("Tag with ID not found"));
	}
